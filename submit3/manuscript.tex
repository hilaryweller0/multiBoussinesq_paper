...
\begin{abstract}
\add[HW]{Traditional parameterizations of convection are a large source of error in weather and climate prediction models and the assumptions behind them are worse as resolution increases.}
...
Some unrealistic assumptions remain such as that convection does not transport mass upwards, convection is in equilibrium with the surroundings and that the variations in the horizontal direction are not relevant. These make
...
It is necessary to assume that operators permute with the averaging \cite<as described by>{TWV+18}. \add[HW]{Terms that appear because the averaging does not commute with the operators can be included in the transfers between fluids.} \remove[HW]{Transfers between fluids are included at this stage} \add[HW]{The equation for $\sigma_i$ is then}:
\begin{equation}
...
\end{eqnarray}
The sub-filter-scale fluxes are represented by constant viscosity diffusion terms for each fluid. The diffusion terms are written so that, in the absence of transfers between fluids and if both fluids are initialized to be identical, the fluids remain identical and $\sigma_i$ acts as a passive tracer:
\begin{eqnarray}
\overline{I_i \nu \nabla^2 \mathbf{u}} &\approx& \sigma_i \nu \nabla^2 \mathbf{u}_i \\
\overline{I_i \alpha \nabla^2 b} &\approx& \sigma_i \alpha \nabla^2b_i .
\end{eqnarray} 
%
The \add[HW]{Boussinesq} pressure \add[HW]{potetials} within each fluid are defined as:
\begin{equation}
...
In order to derive a suitable form of \change[HW]{parameterization}{model} for $p_i$ we will first consider the \citeA{BN86} model of multi-phase flow.

\subsubsection{The \protect\citeA{BN86} model}

\citeA{BN86} proposed a parameterization for pressure differences
in a multi-phase explosion model that has been widely used in mathematics
and engineering for simulating, for example, fluidized beds of granular material \cite<e.g.>{EHM92} and two compressible fluids \cite<e.g.>{SA99}. This model is not used here but is examined in order to derive a parameterization of pressure differences between fluids suitable for atmospheric convection.
...
\label{eq:Pi_div}
\end{equation}
where $\gamma$ is a compressibility with units $\text{m}^2 \text{s}^{-1}$. The ``stabilization'' term reduces to that of \protect{\citeA{TEB19}} in one dimension and the ``correction'' term ensures that $\sum_i \sigma_i p_i =0$. It should be noted that this parameterization destroys \add[HW]{exact} energy conservation. \add[HW]{In future work solving the compressible equations we plan to re-insert the energy lost due to this stabilization as turbulent kinetic energy and heat.}
...
\end{equation}
The Poisson equation for $P^{n+1}$ is found by multiplying each \remove[HW]{eqn}
...
($P_{i}=P+\gamma\sum_j \sigma_j \nabla\cdot\mathbf{u}_j-\gamma\nabla\cdot\mathbf{u}_{i}$), the simulations are more realistic (bottom three rows of figure \ref{fig:multiFluidBubble_500_1}).
...
giving $\gamma=2000\text{m}^{2}\text{s}^{-1}$. \add[HW]{(If scale analysis is used to estimate $L$ and $B$, values within a factor of 10 are calculated, depending on the assumptions.)}
A simulation using
