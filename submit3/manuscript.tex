...
it much simpler to incorporate convection parameterizations into existing models. 
...
\add[HW]{The pressure variations are determined up to a constant in order to ensure the divergence free constraint.}
...
It is necessary to assume that operators permute with the averaging \cite<as described by>{TWV+18}. \add[HW]{Terms that appear because the averaging does not commute with the operators can be included in the transfers between fluids} \remove[HW]{Transfers between fluids are included at this stage} \add[HW]{which take the form}:
\begin{equation}
...
In order to derive a suitable form of parameterization for $p_i$ we will first consider the \citeA{BN86} model of multi-phase flow.
...
S_{10} = w_0\delta.
\end{equation}
Results using two formulations of entrainment are presented in section {\protect\ref{sec:results}}. The first is similar to
