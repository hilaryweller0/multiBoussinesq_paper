...
\add[HW]{The multi-fluids approach has an advantage over ADHOC and CLUBB in solving prognostic equations for first-order moments directly (means of eg. updraft and environment) rather than solving prognostic equations for higher-order moments of the whole fluid in order to diagnose means of updrafts and the environment. Solving directly for sub-fluid means should be less sensitive to closure assumptions.}
...
For the derivation of the conditionally averaged equations we will not consider relabelling of fluid \add[HW]{and so} $I_i$ satisfies:
\begin{equation}
\frac{DI_i}{Dt}=0.
...
We will assume that pressure in each fluid is given by a \remove[HW]{parameterised}
...
may not have the mean properties of the fluid it is leaving \cite<as was assumed by>{WM19}. \change[HW]
...
\change[HW]{The properties of the fluids transferred should depend on the modelling of sub-grid-scale variability which}
...
For two dimensional multi-fluid simulations we \add[HW]{could} assume that the horizontal
...
\end{pmatrix}.
\end{equation}
\add[HW]{However two dimensional multi-fluid simulations are not presented in this paper.}
...
Then the non-dimensional version of (\ref{eq:wi_balances}) is \add[HW]{then}:
\begin{equation}
...
variables. \remove[HW]{It differs from} \citeA{WM19}
\change[HW]{because here we solve the Boussinesq equations and we need stable solutions for equations with a separate pressure in each fluid.}
...
If we include pressure difference between fluids with $\gamma=4\times 10^{4}\text{m}^{2}\text{s}^{-1}$ but
omit entrainment ($S_{ij}=0$) (second row of figure \ref{fig:multiFluidBubble_500_2})
...
\item the pressure difference between fluids is usefully modelled by divergence with a coefficient close to theat expected from scale analysis,
\item a drag term is not useful and
...
of the resolved case between $t=10^{5}\text{s}$ and $2\times10^{5}\text{s}$. The bottom two rows show results of the \change[HW]{single}{one} column multi-fluid model after $10^{4}\ \text{s}$. }
\end{figure}
...
\add[HW]{The two-fluid, one column model of RCE gave results closer to the reference solution than for the rising bubble. This is likely to be because for RCE the rising and falling fluids are averaged over a large area with plumes at different stages of development rather than trying to simulate the complex entrainment pattern behind a single bubble.}

\section{Summary, Conclusions and Further Work \label{sec:concs}}

\add[HW]{This paper demonstrate the plausible utility of using multi-fluid equations to represent sub-grid-scale convection.}
...
fluid of zero buoyancy \add[HW]{is transferred ($b_{ij}^T=0$) also leads to}, unbounded buoyancy. A better model of the buoyancy
of the fluid transferred would rely on more information \add[HW]{of} the
sub-\change[HW]{grid}{filter}-scale variability.
