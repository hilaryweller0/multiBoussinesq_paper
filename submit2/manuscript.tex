...
\add[HW]{The multi-fluids approach has an advantage over ADHOC and CLUBB in solving prognostic equations for first-order moments directly (mean properties of eg. updraft and environment) rather than solving prognostic equations for higher-order moments of the whole fluid in order to diagnose means of updrafts and the environment. Solving directly for sub-fluid means could be less sensitive to closure assumptions.}
...
\note[HW]{Additional material until the start of 2.1}
\begin{eqnarray}
...
The single-fluid Boussinesq equations will be conditionally averaged following \citeA{TWV+18} to give the multi-fluid Boussinesq equations. Lagrangian labels for each fluid $i$ are defined as $I_i=1$ in fluid $i$ and 0 elsewhere. 
...
\end{eqnarray} 
%
\change[WM]{Unlike Thuburn et al. (2018) we define separate pressures for each fluid, $P_i$. The pressures are defined to satisfy:}{The pressures for each fluid, $P_i$, are defined to satisfy:}
\begin{equation}
\overline{I_i \nabla P} = \sigma_i \nabla P_i
\end{equation}
...
{The combination of equations (\ref{eq:BNcompaction}) and (\ref{eq:rhoTransport}) shows that the pressure difference between fluids is related to divergence.}
...
\citeA{TEB19}
\change[WM]{we assume that the pressure perturbation in each fluid is given
by}{and obeying the constraints given above, the pressure in each fluid is chosen to be}:
\begin{equation}
p_{i}=\gamma\sum_j \sigma_j\nabla\cdot\mathbf{u}_j -\gamma\nabla\cdot\mathbf{u}_{i}
\label{eq:Pi_div}
\end{equation}
where $\gamma$ is a \change[HW]{free coefficient}{compressibility with units $\text{m}^s \text{s}^{-1}$. It should be noted that this parameterisation destroys energy conservation.}
...
If
\change[WM]{we include pressure difference between fluids}
...
\change[HW]{an unrealistic discontinuity with no}
...
\change[HW]{This combination produces smooth solutions}
...
\change[HW]{very high buoyancy in fluid one which is not present in the resolved simulation.}
