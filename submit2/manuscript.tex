...
\change[HW]{assumed that all fluids share the same pressure.}
...
Two vertical slice test cases are \change[HW]{used}{developed} to \add[HW]{tune parameters and to} \change[HW]{validate}{evaluate} the \change[HW]{multi}{two}-fluid \change[HW]{model}{equations for representing dry, two-dimensional, sub-grid-scale convection}; a buoyant rising bubble and radiative convective equilibrium. \add[HW]{These}
...
\add[HW]{The multi-fluids approach has an advantage over ADHOC and CLUBB in solving prognostic equations for first-order moments directly (mean properties of eg. updraft and environment) rather than solving prognostic equations for higher-order moments of the whole fluid in order to diagnose means of updrafts and the environment. Solving directly for sub-fluid means should be less sensitive to closure assumptions.}
...
equations \remove[HW]{for multiple segments} and showed how this is a generalisation
...
\change[HW]{Multi-fluids promises big improvements
...
However, there is a long way to go, particularly as multi-fluids}
...
and sustained pressure differences between the fluids (section \ref{sec:results}) \add[HW]{and that the use of separate pressures stabilises the equations}. 
...
and Buossinesq equations. Therefore\change[WM]{ for simplicity we focus on the}{, the}
...
The single-fluid Boussinesq equations will be conditionally averaged following \citeA{TWV+18} to give the multi-fluid Boussinesq equations. Lagrangian labels for each fluid $i$ are defined as $I_i=1$ in fluid $i$ and zero elsewhere. 
For the derivation of the conditionally averaged equations \change[WM]{we will not consider relabelling of fluid}{the relabelling of fluids will not be considered,} \add[HW]{and so} $I_i$ satisfies:
\begin{equation}
\frac{DI_i}{Dt}=0.
\label{eq:LagrangianLabel}
\end{equation}
\change[WM]{Applying conditional volume average to $I_i$ we define the volume fraction of fluid $i$ and the velocity and buoyancy of fluid $i$}{By applying a conditional volume average (denoted by the overbar), the fluid $i$ volume fractions, velocities and buoyancies are defined by}:
\begin{eqnarray}
...
\label{eq:sumOne}
\end{equation}
\change[WM]{As we are considering the Boussinesq equations we assume}{As the system is governed by the Boussinesq equations, it is assumed} that variations in density only affect the \change[WM]{buoyancy terms}{definition of the buoyancy terms (table \protect\ref{tab:defns})} so $\mathbf{u}_i$ and $b_i$ are defined without density weighting. 

To derive a transport equation for $\sigma_i$\change[WM]{ we combine (\protect\ref{eq:LagrangianLabel})}{, (\protect\ref{eq:LagrangianLabel}) is combined} with $I_i$ times (\ref{eq:singleDivFree}) and volume average. After permuting operators with the averaging \cite<as described by>{TWV+18}\add[WM]{,} and using the definitions of $\sigma_i$ and $\mathbf{u}_i$\add[WM]{,} \remove[WM]{we get} the evolution equation for $\sigma_i$\add[WM]{ is obtained}:
\begin{equation}
...
Next \change[WM]{we will consider conditionally averaging the terms of }{the conditional averaging of terms in} the momentum (\ref{eq:singleMom}) and buoyancy (\ref{eq:singleb}) equations\add[WM]{ is considered}. Conditionally averaging the advection terms leads to sub-filter-scale fluxes, $F_{SF}$, since the velocity and buoyancy are not uniform within each fluid at the filter scale:
\begin{eqnarray}
...
\change[WM]{Lots of}{Many} terms appear when attempting to conditionally average the diffusion terms which we do not know how to parameterise. \change[WM]{Therefore we will use simply}{The conditionally averaged diffusion terms are therefore assumed to behave like}:
\begin{eqnarray}
\overline{I_i \nu \nabla^2 \mathbf{u}} &=& \sigma_i \nu \nabla^2 \mathbf{u}_i \\
\overline{I_i \alpha \nabla^2 b} &=& \sigma_i \alpha \nabla^2b_i .
\end{eqnarray} 
%
\remove[WM]{Unlike \protect\citeA{TWV+18}} \change[WM]{we define separate pressures for each fluid, $P_i$. The pressures are defined to satisfy:}{The pressures for each fluid, $P_i$, are defined to satisfy:}
\begin{equation}
\overline{I_i \nabla P} = \sigma_i \nabla P_i
\end{equation}
\add[WM]{which differs from \protect\citeA{TWV+18} who use a shared fluid pressure.}
%
Combining \change[WM]{these}{the above terms} gives the conditionally averaged momentum and buoyancy equations:
\begin{eqnarray}
...
\change[HW]{assumed that each co-located fluid shared the same pressure leading.}
{used the same pressure for both fluids.}
...
if \change[WM]{we assume}{one assumes} that $\mathbf{V}=\sum_{i}\sigma_{i}\mathbf{u}_{i}$ and
...
\change[WM]{If we assume that $\mathbf{V}=\mathbf{u}_{i}$ for each fluid then
we get $\mu p_{i}=-\sigma_{i}\nabla\cdot\mathbf{u}_{i}$.}{Instead assuming $\mathbf{V}=\mathbf{u}_{i}$ for each fluid results in $\mu p_{i}=-\sigma_{i}\nabla\cdot\mathbf{u}_{i}$.}
...
\add[WM]{then} this is equivalent to the diffusion term in one dimension. However
...
\citeA{TEB19}\change[WM]{ we assume that the pressure in each fluid is given
by}{, the pessure in each fluid is chosen to be}:
\begin{equation}
p_{i}=-\gamma\nabla\cdot(\sigma_i \mathbf{u}_{i})
\label{eq:Pi_div}
\end{equation}
where $\gamma$ is a \change[HW]{free coefficient}{compressibility with units $\text{m}^s \text{s}^{-1}$. 
This parameterisation of $p_i$ has $\sigma_i$ inside the divergence in order to ensure that $\sum_i p_i =0$ and therefore that $\sum_i P_i = P$ which makes the equations more striaghtforward to solve.}
\add[HW]{However it should be noted that this parameterisation destroys energy conservation.}
...
\add[HW]{The transfers between fluids are related to entrainment, $\varepsilon$, and detrainment, $\delta$. Fluid 0 is the environment and fluid 1 is updrafts implying:}
\begin{equation}
S_{01} = w_1\varepsilon, \;\;\;\;\;\;\;
S_{10} = w_0\delta.
\end{equation}
\add[HW]{Results using two formulations of entrainment are presented in section {\protect\ref{sec:results}}. The first is}
...
may not have the mean properties of the fluid it is leaving \cite<as was assumed by>{WM19}. \change[WM]{In fact, the most buoyant air should be transferred from fluid 0 to
fluid 1 and vice versa, and the air with least downward momentum
should be transferred from 0 to 1 and vice versa.
The properties of the fluids transferred should depend on the modelling of sub-grid-scale variability which
is beyond the scope of this paper.}{A realistic estimate of the properties of the fluid transferred can only be based on knowledge of the sub-filter-scale variability (i.e. the variability within each fluid). However, modelling of sub-filter-scale variability is beyond the scope of this paper.}

\change[WM]{In section 4
...
Therefore the air that is transferred at the interface will have $w=0$.}{Since conditional averaging is based on the vertical velocity in this study, the sub-set of fluid 0 that is about to rise (i.e. within the next timestep) should be transferred from fluid 0 to fluid 1, and vice versa for the fluid about to fall in fluid 1. The fluid transferred will pass through the fluid interface and will therefore have a vertical velocity of $w=0$.}
...
\item There are two fluids, \change[HW]{$i$ and $j$}{0 and 1.}
...
\tilde{z} & = & z/L
\end{eqnarray*}
\add[HW]{where $\sigma$ is a typical value of $\sigma_i$ which must be $O(1)$ for one of the fluids.}
...
\change[WM]{because here we solve the Boussinesq equations and we need stable solutions for equations with a separate pressure in each fluid.}
...
\change[HW]{The first equation to be solved each outer iteration is eqn}
...
converge \change[HW]{and}{implying} $p_{i}^{\ell}=p_{i}^{n+1}$. 
...
\add[HW]{inviscid models ($\nu=0$, $\alpha=0$)} but similar solutions are obtained solving Boussinesq equations.
...
\change[HW]{The solution of the  rising buoyant bubble solving the Boussinesq equations for 1000s}
{The solution of the Boussinesq equations in two dimensions for the rising bubble at high resolution at the initial time, 500 seconds and 1000 seconds}
is shown in figure \ref{fig:bubble} with buoyancy coloured,
pressure contoured and the $w=0$ contour dotted. The solution is
very similar to the fully compressible solution \cite{BF02}.
...
\change[HW]{Horizontally and conditionally averaged fields, conditioned on $w$, are}
...
the pressures of both fluids are assumed equal \add[HW]{meaning that} the multi-fluid
equations \change[HW]{are}{should be} unstable \cite{TEB19}. The simulation with ($S_{ij}=0$)
...
other.
\add[HW]{The two fluids have mixed due to the two way stabilising transfers which has reduced the buoyancy in the rising fluid and increased the buoyancy in the sinking fluid.}
...
($P_{i}=P-\gamma\nabla\cdot\sigma_i\mathbf{u}_{i}$), \add[HW]{the} simulations are more realistic (bottom three rows of figure \ref{fig:multiFluidBubble_500_1}),
\add[HW]{all with a realistic $\sigma_1$ profile}.
...
radius, a buoyancy scale equal to the initial maximum \add[HW]{initial} buoyancy 
\add[HW]{and $\sigma=1$}
to give $\gamma=2.3\times10^{4}\text{m}^{2}\text{s}^{-1}$. This $\gamma$
(third row of figure \ref{fig:multiFluidBubble_500_1}) makes
the velocity profile
\change[HW]{too smooth and the updraft too weak so the
bubble does not rise enough.}
...
\change[HW]{It is reasonable to consider smaller
values of $\gamma$ because the buoyancy does not retain its initial
maximum for very long. Using $\gamma=10^{4}\text{m}^{2}\text{s}^{-1}$ gives more accurate solutions whereas using $\gamma=4\times10^{3}\text{m}^{2}\text{s}^{-1}$
leads to the leading edge of the bubble rising too quickly.}
...
Smaller values of $\gamma$ leave a discontinuity in the updraft velocity at the leading edge of the bubble and larger values of $\gamma$ reduce the velocity further \remove[HW]{meaning that the bubble does not rise enough} (not shown). None of the values of $\gamma$ reproduce the \change[HW]{peak updraft velocities but $\gamma=10^{4}\text{m}^{2}\text{s}^{-1}$ comes closest, meaning that the bubble rise and $\sigma_{1}$ are approximately correct and}{pressure dip at the trailing edge of the bubble but the }
pressure increases above the centre of the bubble are realisitic.
\remove[HW]{The large low pressure at the bottom of the bubble is not reproduced. This is located at the lobes of the highest buoyancy in the resolved simulation (figure {\protect\ref{fig:bubble}}). It would be tempting to model this by assuming hydrostatic pressure or the Bernouilli equation, but these are both forms of the momentum equation which is used to calculate the velocity and so cannot also be used to model the pressure.}

The results in the top row of figure \ref{fig:multiFluidBubble_500_2}
use
\change[HW]{$\gamma=-10^{4}$ and $S_{ij}=-\nabla\cdot\mathbf{u}_{i}$ }
{divergence transfer and pressure differences between fluids but, different from previous simulations, also use}
...
\change[HW]{which is accounted for in the entrainment
term of the momentum equation.}
{These are already accounted for in the pressure difference and entrainment parameterisations}
so adding a separate term for drag risks double counting and slows the plume too much. 
...
If \change[WM]{we include pressure difference between fluids}{pressure differences between fluids are included} with $\gamma=4\times 10^{4}\text{m}^{2}\text{s}^{-1}$ \change[WM]{but
omit}{without} entrainment ($S_{ij}=0$) (second row of figure \ref{fig:multiFluidBubble_500_2})
the fluids move past each other but there is an unrealistic discontinuity with \change[HW]{no}{very little} fluid 1 below the \change[HW]{buoyant fluid}{bubble.}
...
rate, $\varepsilon=0.2/r_{c}$. This is used \change[HW]{combined}{in combination with} with pressure
differences between fluids in the third row of figure \ref{fig:multiFluidBubble_500_2}.
This combination produces smooth solutions but the \add[HW]{rising volume fraction is not right, the} updraft is too
weak and too much is entrained into fluid 1. Using this entrainment,
the updraft will entrain more and more air until $\sigma_{1}=1$.
The $S_{ij}=-\nabla\cdot\mathbf{u}_{i}$ model gives
\change[HW]{much more accurate entrainment and detrainment for this test case using conditional averaging based on $w$.}
{results closer to the reference solution conditionally averaged based on $w$.}
...
\change[HW]{These solutions are similar to the resolved solutions but the bubble has moved a little too fast.}
{This again leads to very large buoyancy in fluid one.}
\remove[HW]{These simple models reinforce
...
These simple assumptions can lead to unrealistic behaviour. The best model tested is $b_{ij}^{T}=b_{i}$ which assumes that the fluid transferred takes the mean properties with it.}

\remove[HW]
{This section has demonstrated the value of using single column, multi-fluid model to simulate a buoyant rising bubble with a single
updraft fluid and a single downdraft fluid. Transfers between
the fluids have been successfully modelled using entrainment set by divergence giving realistic updraft fractions. Entrainment calculated
...
with a coefficient close to that expected from dimensional analysis. Including drag between the fluids did not improve simulations.
Two models of transferring non-mean properties between fluids were tested
but neither was plausible}.

\begin{figure}
\noindent
\includegraphics[width=\linewidth]{figures/multiFluidBubble_1000.pdf}
\caption{\label{fig:multiFluidBubble_1000} Results of the rising bubble at $t=1000\ \text{s}$. Solid lines are
the multi-fluid, \change[HW]{single}{one} column solutions of the rising bubble. Dashed
lines are the single-fluid, resolved solutions.
}
\end{figure}
...
leads to
\change[HW]{convecting and non-convecting regions}
{rising and falling fluids} with area fractions,
$\sigma_{i}$, which are approximately uniform with height. Models
for entrainment and detrainment into and out of \change[HW]{convecting}{these} regions
...
\change[HW]{However sub-grid-scale variability
...
velocity will be transferred to the rising fluid and vice versa. An accurate estimate of these would depend on knowledge of the sub-grid-scale
...
which is consistent with our conditional averaging.}
