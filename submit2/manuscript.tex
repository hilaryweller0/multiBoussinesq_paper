...
Two vertical slice test cases are used to \change[HW]{validate}{evaluate} the \change[HW]{multi}{two}-fluid
model; a buoyant rising bubble and radiative convective equilibrium. \add[HW]{These}
are simulated at high resolution and conditionally averaged based
on the sign of the vertical velocity. The \change[HW]{multi}{two}-fluid model reproduces
...
transferred should depend on the modelling of \change[HW]{sub-grid-scale}{sub-filter-scale} variability \add[HW]{(i.e. the variability within each fluid)}
...
\section{Numerical \change[HW]{Solution}{Method} \label{sec:numerics}}
...
\section{\change[HW]{Validation}{Evaluation} Test Cases\label{sec:results}}
...
divergence giving realistic updraft fractions. Entrainment calculated
...
at coarse horizontal resolution. Two test cases are used to \change[HW]{validate}{evaluate}
